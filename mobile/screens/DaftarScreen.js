...
export default function DaftarScreen({ navigation }) {
  ...
  const muatHalaman = useCallback(
    async (h, { reset = false } = {}) => {
      ...
      setGalat(null);
      try {
        const json = await ambilDaftarKonser(h);
        const baris = Array.isArray(json) ? json : json.items || json.data || [];
    ...
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.info}>
          Cek BASE_URL di mobile/config.js dan pastikan API + HP satu jaringan.
        </Text>
        <Pressable
          style={styles.btn}
          onPress={() => {
  ...
  return (
    <FlatList
      ...
      onEndReached={() => {
        if (!memuat && !habis) muatHalaman(halaman + 1);
      }}
      ListHeaderComponent={
        <Text style={styles.heading}>Open Concert · {data.length} dimuat</Text>
      }
      ListFooterComponent={
        memuat ? (
...
const styles = StyleSheet.create({
  ...
  stock: { color: colors.accent, fontSize: 12, marginTop: 2, fontWeight: "600" },
